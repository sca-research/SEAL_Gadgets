...
class Cal_SNR(object):
    def __init__(self, mask_order=1, d=1, name_trs_file="pini2_2", gadget_name="pini2_2", points_per_clock=125):
        ...
        # self.path_trace = "/media/mahpar/Expansion/Nima/traces/trace_" + gadget_name + "/trace_" + gadget_name + "_" + str(
        #     self.mask_order + 1) + "_shares/"
        self.path_trace = "/media/IWAS\mahpar/Expansion/Nima/traces/trace_isw/isw_rnd/V2/"
        self.path_trace = "/media/IWAS\mahpar/Expansion/Nima/traces/trace_isw/isw_f_test/step_1_2/"
        # self.path_trace = "/media/IWAS\mahpar/Expansion/Nima/traces/trace_dom_indep/trace_dom_indep_3_shares/"
    ...
    def comput_snr_for_random_traces(self, n_rnd_tr):
        rang_ = self.n_t
        print(f"--- randomly select {n_rnd_tr} traces from {self.n_t} traces")
        rand_list = np.random.choice(rang_, n_rnd_tr, replace=False)

        # [t, in_data, out_data] = self.trs.extract_trace_sets()
        [t, in_data, out_data] = self.trs.get_randomly_traces_data(list_ind=rand_list)
        # t = centring_trace(t)
        # t  = t[:, 70*125: 71*125]
        ...
        for j, (key, value) in enumerate(dic_im.items()):
            ...
            threshold = 0.125  # in SNR for selecting poi
        ...
        return [list(poi), list(cycles_of_poi)]
...
if __name__ == "__main__":
    ...
    # out_s.comput_snr()
    name_trs = "ttest_isw3_20000K_27cy_24_250_240cy"
    # name_trs = str(sys.argv[1])
    out_s = Cal_SNR(mask_order=2, d=1, name_trs_file=name_trs, gadget_name="isw", points_per_clock=125)
    print("***********************************************************")
    out_s.comput_snr_for_random_traces(20000)
    print("***********************************************************")
    out_s.comput_snr_for_random_traces(30000)
    print("***********************************************************")
    out_s.comput_snr_for_random_traces(40000)
    print("***********************************************************")
    out_s.comput_snr_for_random_traces(50000)
    #
    #
