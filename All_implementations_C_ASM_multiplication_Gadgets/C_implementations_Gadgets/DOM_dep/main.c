...
int rnd_n = (Mask_ORD+1) + (Mask_ORD * (Mask_ORD+1) /2); // The number of randomness in DOM_dep multiplication gadget
...
int main(void)
{
    ...
    for (int i = 0; i < 100; i++)
    {
        ...
        static uint8_t rnd_Dom_dep[(Mask_ORD+1) + (Mask_ORD * (Mask_ORD+1) /2)]; // Random number for Dom_dep multiplication
        ...
        /*
        DOM_dependent(INPUT: a[Mask_ORD+1], INPUT: b[Mask_ORD+1], INPUT: rnd[(Mask_ORD+1) + (Mask_ORD * (Mask_ORD+1) /2)], OUTPUT: c[Mask_ORD])
        c = a * b
        rnd_Dom_dep: random numbers (on_the_fly)*/
        DOM_dependent(a, b, rnd_Dom_dep, c);
