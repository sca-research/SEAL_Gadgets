...
int main(void)
{
    // The number of randomness in Bbpp_Mult multiplication gadget
    if (Mask_ORD % 2 == 0){
        int rnd_n = (Mask_ORD * (Mask_ORD)) /4 + Mask_ORD;
    }
    else{
        int rnd_n = ( (Mask_ORD * (Mask_ORD)) -1) /4 + Mask_ORD;
    }
    ...
    for (int i = 0; i < 100; i++) {
        ...
        static uint8_t rnd_gadget[(Mask_ORD * (Mask_ORD)) /4 + Mask_ORD]; // Random number for Bbpp_2_Mult multiplication
